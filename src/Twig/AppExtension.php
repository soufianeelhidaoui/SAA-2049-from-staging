...
class AppExtension extends AbstractExtension
{
    ...
    public function addToCache($url)
    {
        ...
        if (!file_exists($key)) {
            ...
            file_put_contents(__DIR__ . '/../../public/media/cache/gmap/' . $key, file_get_contents($url));
        }
    ...
    public function imgUrl($filepath, $size): string
    {
        ...
        $s3ImageUrl = $this->s3ImageUrl($filepath);

        return $s3ImageUrl;
    }

    private function s3ImageUrl($path): string
    {
        $awsS3Bucket = $_ENV['AWS_BUCKET_NAME'] ?? getenv('AWS_BUCKET_NAME');
        $awsS3Folder = $_ENV['AWS_BUCKET_FOLDER'] ?? getenv('AWS_BUCKET_FOLDER');
        $path = ltrim($path, '/');
        $prefix = 'media/image/media/rich-editor';
        $s3Url = "https://{$awsS3Bucket}.s3.amazonaws.com/{$awsS3Folder}/media/image/{$path}";
        // Check if the URL contains duplication
        if (strpos($s3Url, $prefix) !== false) {
            // If it does, remove it from the URL
            $s3Url = str_replace('media/image/', '', $s3Url);
        }
        return $s3Url;
    }
