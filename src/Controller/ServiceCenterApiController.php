...
class ServiceCenterApiController extends AbstractController
{
     ...
     * @return JsonResponse
     */
    public function updateOrder(string $service_center_id, int $order_id, Request $request, OrderRepository $orderRepository, EntityManagerInterface $entityManager){
    ...
    public function updateTerms(string $service_center_id, Request $request, KernelInterface $kernel){
        ...
        if( !$file = $form->get('pdf')->getData() )
            return $this->json(['error'=>'File not found'], 500);

        if( !$file->move($termDir, $service_center_id.'.pdf') )
    ...
    public function getTerms(string $service_center_id, KernelInterface $kernel){
        ...
        $terms_file = $priceDir.$service_center_id.'.pdf';
