...
export default {
	...
	add(product_id, variant, quantity, token) {
      ...
      if( variant )
        params.cartItem.variant = variant;
			...
			Vue.http.post(shop.routes.root+'/cart/add.js?productId='+product_id, {sylius_add_to_cart:params}).then(response=>{
	...
	change(line, quantity) {
		...
		return new Promise((resolve, reject) => {
			...
			Vue.http.post(shop.routes.root+'/cart/change.js', params).then(response=>{
